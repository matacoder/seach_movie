...
class MockRestClient(RestClient):
    def _get_response(self, endpoint, **params):
        ...
        status_code = 200
        response = MockResponse(text, status_code).json()
        no_found_response = MockResponse(text_no_results, status_code).json()
        return (
            response
            if params["term"] == "Star Trek: Generations"
            else no_found_response
        )
